...
dashboard = replaceOnce(
  ...
  'reportPeriod === "daily" ? "Bugungi baholashlar bo‘yicha" : "Oxirgi 30 kunning kunlik natijalari o‘rtachasi"',
  "monthly description",
);
...
dashboard = replaceOnce(
  ...
  'reportPeriod === "daily" ? "bugungi baholash" : "oxirgi 30 kun bo‘yicha"',
  "monthly metric copy",
);

fs.writeFileSync(dashboardPath, dashboard, "utf8");
console.log("PATCH31: public dashboard monthly copy now says rolling 30-day average.");
...
const newMonthly = `    const evaluationPromise = period === "monthly"
      ? db.pool.query(
          \`SELECT institution_id AS "institutionId",
                  criterion_id AS "criterionId",
                  (SUM(score)::float8 / 30.0) AS score
             FROM daily_evaluations
...
route = replaceOnce(route, oldMonthly, newMonthly, "rolling 30-day query");

route = replaceOnce(
  route,
  '{ institutions, evaluations, period, currentDate }',
  '{ institutions, evaluations, period, currentDate, averagingDays: period === "monthly" ? 30 : 1 }',
  "averaging metadata",
);

fs.writeFileSync(routePath, route, "utf8");
console.log("PATCH31: monthly mode now averages each day's KPI over the latest 30 calendar days; days without a score count as zero.");
