...
// Railway/proxy can rewrite or report Content-Length differently from the bytes
// finally delivered to Next.js. Content-Length is therefore diagnostic only.
// Integrity is enforced by the browser SHA-256 checksum, received bytes, 30 MB
// limit and file-signature validation. If prepare size differs but checksum is
// valid, synchronize the upload intent to the verified received byte length.
replaceOnce(
...
console.log("PATCH27: file uploads now rely on SHA-256 integrity instead of proxy Content-Length; valid ZIP/RAR/Office/PDF uploads up to 30 MB are accepted securely.");
